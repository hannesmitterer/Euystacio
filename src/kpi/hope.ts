...
const DEFAULT_RATIO_FOR_PURE_HOPE = 2; // When sorrow is 0 but hope > 0, use this ratio
...
export function getRollingKpi() {
  ...
  const ratio = avgSorrow > 0 ? avgHope / avgSorrow : (avgHope > 0 ? DEFAULT_RATIO_FOR_PURE_HOPE : 0);
